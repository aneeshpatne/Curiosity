...
async def follow_up(
    ...
    if search_type == "deep":
        ...
        links = await get_links(question, max_results=20)
        # Call deep_search with provided links to avoid duplicate search, ensuring nested calls emit their sources as well.
        _ = await deep_search(question, depth=2, sid=sid, msg_id=msg_id, emit_status=emit_status, emit_sources=emit_sources, links=links)
